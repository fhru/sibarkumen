...
const createSPPBSchema = z.object({
  spbId: z.number(),
...
const completeSPPBSchema = z.object({
...
export async function completeSPPB(
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      // 2. SPB status is already SELESAI from creation, no need to update again unless we want to "ensure" it.
      // Keeping it safe or removing?
      // User request said "saat sppb nya sudah dibuat", so it's done at creation.
      // I will comment it out or remove it to avoid confusion/redundancy, or leave it as a safety check?
      // "Safety check" is fine but if logic changes later, we have 2 places.
      // Let's remove it for clarity as per instruction.
